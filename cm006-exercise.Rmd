...
```{r}
# compare
select(gapminder, cont = continent, everything())  # this renames it but also alters the order

rename(gapminder, cont = continent)  # rename and maintain position

# note the the left hand side is the new name (left = leave) and right hand side is original name (right = remove)
```
...
```{r}
arrange(gapminder, year)  # deafault is ascending (least to greatest)
```

2. Order by year, in descending order.

```{r}
arrange(gapminder, desc(year))  # 'desc' orders in decsending order
```
...
# if you want to order in decsending order, wrap each in the 'desc' function:
...
arrange(select(gapminder, year, lifeExp, country), year, lifeExp)
# nesting is effective but can be difficult to follow after executing the code (like Excel nesting)

gapminder %>% 
  select(year, lifeExp, country) %>% 
  arrange(year, lifeExp)

# this is nice and tidy because you can see what is happening and the logical progression of commands
```
...
```{r}
gapminder %>%
  transmute(gdpBill = round(gdpPercap*pop/10^9, digits = 2))

# only keeps the mutated variable
```
