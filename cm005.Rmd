...
| scale                 | linear |
| statistical transform | count (group by continent) |
...
ggplot(mauna, aes(y = month)) +
  ...
  labs(y = paste((expression(CO[2])), "(ppm)"), x = "")
```
...
p + geom_point(colour = "green", size = 1)
```
...
ggplot(gapminder) +
  ...
  # x-axis scale has the data compressed near x=0, plot data on log scale
...
ggplot(gapminder) +
  ...
  scale_x_log10("GDP per capita", labels = scales::dollar_format()) +
  ylab("Life Expectancy (yrs)") +
  # convert to polar coordinate system / plot as a windrose/polar plot
  coord_polar() 
```
