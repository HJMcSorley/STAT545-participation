...
Function `case_when()` (dplyr) is a tidier way to vectorise multiple `if_else()` (Base R) statements. you can read more about this function [here](https://dplyr.tidyverse.org/reference/case_when.html).

```{r create new variable with levels}
gapminder %>% 
  filter(year == 1997) %>% 
  mutate(life_level = case_when(lifeExp < 23 ~ "very low",
                                ...
                                lifeExp < 59 ~ "moderate",
                                lifeExp < 70 ~ "high",
                                TRUE ~ "very high")) %>%   # this is like the "else" part of if_else()
...
- Notice also how levels on x-axis are placed in the "wrong" order (alphabetical).
...
# do the same, but specify that 'life_level' is a factor
gapminder %>% 
  filter(year == 1997) %>% 
  mutate(life_level = factor(case_when(lifeExp < 23 ~ "very low",       # specify 'factor' 
                                lifeExp < 48 ~ "low",
                                lifeExp < 59 ~ "moderate",
                                lifeExp < 70 ~ "high",
                                TRUE ~ "very high"),
                             levels = c("very low", "low", "moderate", "high", "very high"))) %>%
  # specify ranking of levels within the factor() function
  ggplot() + geom_boxplot(aes(x = life_level, y = gdpPercap)) +   
  # must add factor to ggplot
  labs(y = "GDP per capita, $", x= "Life expectancy level, years") +
...
# use sample_n() to select 5 countries
(h_countries <- sample_n(gapminder, size = 5))
names(h_countries$country)
...
For instance ,
    `  
```{r}
# fct_infreq() re-orders by frequency 
gapminder %>%
  ggplot() +
  geom_bar(aes(forcats::fct_infreq(continent))) +  # fct_infreq() feeds into aes()
  coord_flip() +
  theme_bw() +
  ylab("Number of entries") + xlab("Continent")
```
...
```{r}
##  default summarizing function is median() -- here, switch to min
gapminder %>%
  ggplot() +
  geom_bar(aes(fct_reorder(continent, lifeExp, min))) + # which column and reorder based on what
  coord_flip()+
  theme_bw() +
...
```{r}
# recode -- this is useful for renaming defaults, shortening, clarifying
gapminder %>%
  ggplot() +
  geom_bar(aes(fct_recode(continent, "OCN" = "Oceania", "AMR" = "Americas"))) + 
  coord_flip()+
  theme_bw() +
  xlab("Continent") + ylab("Number of entries") 
