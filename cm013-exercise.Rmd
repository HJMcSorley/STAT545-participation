...
1. easy to see (e.g. axes font size, colours/contrast)
2. self-explanatory axes labels
3. attractive aesthetic (not ugly) 
4. good caption (describe the data and plot layout)
5. interesting data to plot (e.g logical and engaging)
...
* This graph doesn't indicate motivation, 
* Romania appears to work the longest hours relative to other EU nations
* what is the sample size, standard deviation?
* the difference between high/med/low is very small (relative standard deviation is small) --difference may not be significant
* the x-axis scale is wierd
 

- Main takeaway: axes are important, the work hours is roughly the same for all the nations listed and this may not be the best layout to plot the story the authors were trying to tell
...
- __Answer__: Do not know (the y-axis scale is not helpful, no trendline or standard deviation)

* no numbers on x-axis (what is the data range)
* need SD and narrower y-axis - don't start at zero here (if the change is 2 degrees, y-axis should span maybe 10 degrees)
* y-axis 

- Main takeaway: Don't blindly follow plotting advice (e.g. start axis at zero), think about the data and chose axes appropriately.
...
* colour choice -- red background
* y-axis is in reverse!!!!
* 

- Main takeaway: Do not get creative with your axes -- plot simply to get to the point, you may end up confusing things and mis-representing the data if you get too creative with colours and axes.

___check out: https://callingbull.org/___
...
check out this website: https://plot.ly/online-chart-maker/

* sign up for studio cloud 
* copy 2 lines of code to connect to online (API for online plotting) 
...
# same thing but with plot_ly syntax
gapminder %>% 
  plot_ly(x = ~gdpPercap,
         y = ~lifeExp,
         colour = ~continent,
         ...
         type = 'scatter',
         mode = 'markers')
# this didn't quite work. Warning messages
```
...
* the comparative axes (primary and secondary y-axes) differ by 2 orders of magnitude 
  * the relative change between these two variables is negligible (10% change to 0.6% change)
* the context of the plot for scientists is different that the context for general public  

(gender gap in the 100m dash)

* poor extrapolation methods -- when people will be finishing races in negative time

### Case Study 2: A case for pie charts 

* they're not so bad, but don't use them -- out of time
...
1. simple background, clean colours
2. remove redundant words - less is more
3. lighten lines, remove distracting borders
4. 3D effect, shadowing (special effects are generally terrible)
5. think about the journal/audience for requirements
