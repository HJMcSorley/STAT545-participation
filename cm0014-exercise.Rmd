...
Concept of "anscombe's quartet" -- do not blindly trust summary statistics.

```{r}
suppressPackageStartupMessages(library(tidyverse))
library(gapminder)
library(broom)  # tidy tables of stats analysis
```
...
Note: tilda indicates dependency, so y ~ x says y is dependent on x (independent variable)
...
```{r}
(my_lm <- lm(formula = lifeExp ~ year, 
             data = gapminder_France))
```
Does that mean that the life expectency at "year 0" was equal to -397.7646?!

We are interested in the modeling results around the modeling period which starts at year __1952__. To get a meaniningful "interpretable" intercept we can use the `I()` function.
```{r}
# use new function I()
# creates a new variable on the fly
(my_lm <- lm(formula = lifeExp ~ I(year - 1952),
             data = gapminder_France))
```


Note: residuals are the differences between data point and the line of best fit. You hope to reduce the absolute magnitude of residuals. They can be helpful in identifying systematic errors in your data.

Try plotting the residuals
```{r residuals}
# plot data
ggplot(gapminder_France, aes(x = year, y = lifeExp)) +
  geom_point()
...
# years are now counts from start of dataset (e.g., 1 = 1952)

# if you want to focus on only a portion of the data...
# create a new datafram
gapminder_France2 <- data.frame(year = seq(2000, 2005))

# pass subset dataframe to predict()
...
Poverty, gang violence, prevalence of HIV AIDS all contributed to a stark drop in life expectancy in Zimbabwe in the 1990's.
...
```
This does not look right, logically you know this model will generate erroneous predictions. 
...
ggplot(gapminder_Zimbabwe, aes(x = year, y = lifeExp)) + 
  ...
  geom_smooth(method = "lm", 
              formula = y ~ poly(I(x - 1952), degree = 2))  # polynomial degree 2 (parabolic)
```
