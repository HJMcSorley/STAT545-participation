...
(lotr_wide <- lotr %>% 
  pivot_wider(id_cols = c(-Race, -Words), 
              names_from = Race, 
              values_from = Words) %>% 
  rename(Human = Man))  # 'Man' is not a race, replace with 'Human'
```
...
(guest_long <- guest %>% 
  pivot_longer(cols      = c(-party, -name), 
               names_to  = c(".value", "event"),  # ".value" maintains that portion as a column
               names_sep = "_"))
```
...
tibble(label = "Sample A",
             x = rnorm(n),     # rnorm generates random variables
             y = x + rnorm(n))

# this wouldn't work if you were building a dataframe
# tibble (and dplyr in general) can work sequentially within a function
```
...
experiment %>% complete(nesting(name, trt), rep)  # complete turns missing values that are implicit to explicit
# nesting groups treats them like a set?
